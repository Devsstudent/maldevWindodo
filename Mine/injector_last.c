...
int main(int ac, char **av) {
    ...
    char *name = av[1];
    char payload[] = {0xc3, 0xff, 0xc3, 0xff, 0xc3};
    HANDLE *hFile;
    ...
    DWORD dwNewFileSize = dwFileSize + sizeof(payload);
...
//    PIMAGE_SECTION_HEADER pLastSection = (PIMAGE_SECTION_HEADER) ((PUCHAR)pSectionHeader + (pNtHeader->FileHeader.NumberOfSections - 1) * sizeof(IMAGE_SECTION_HEADER))
    ...
    memcpy(dstCpy, payload, sizeof(payload));

    pLastSection->Characteristics |= IMAGE_SCN_MEM_READ | IMAGE_SCN_MEM_EXECUTE;
    pLastSection->SizeOfRawData += sizeof(payload);
    pLastSection->Misc.VirtualSize += sizeof(payload);
